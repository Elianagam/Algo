...
bool pila_apilar(pila_t *pila, void* valor){
	if (pila->cantidad == pila->capacidad){
		if (! pila_redimensionar(pila,pila->capacidad * FAC_REDIMENSION)){
			return false;
		}
	}
...
void* pila_desapilar(pila_t *pila){
	...
	if (pila->cantidad < pila->capacidad / FAC_REDIMENSION){
		pila_redimensionar(pila,pila->capacidad / FAC_REDIMENSION);
	}
	pila-> cantidad--;
